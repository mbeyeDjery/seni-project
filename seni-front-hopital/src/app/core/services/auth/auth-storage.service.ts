...
import {SENI_HOPITAL_AUTH_TOKEN, STORE_URL} from "../../utils/constants";
...
export class AuthStorageService {
  ...
  storeAuthenticationToken(authResponse: IAuthResponse, rememberMe: boolean): void {
    ...
    if (rememberMe) {
      this.localStorageService.setItem(SENI_HOPITAL_AUTH_TOKEN, JSON.stringify(authResponse));
    } else {
      this.sessionStorageService.setItem(SENI_HOPITAL_AUTH_TOKEN, JSON.stringify(authResponse));
    }
  }

  getAuthenticationToken(): IAuthResponse | null {
    const authenticationToken = this.localStorageService.getItem(SENI_HOPITAL_AUTH_TOKEN) ?? this.sessionStorageService.getItem(SENI_HOPITAL_AUTH_TOKEN);
    return authenticationToken ? convertToAuthResponse(JSON.parse(authenticationToken)) : null;
  }
  clearAuthenticationToken(): void {
    this.localStorageService.removeItem(SENI_HOPITAL_AUTH_TOKEN);
    this.sessionStorageService.removeItem(SENI_HOPITAL_AUTH_TOKEN);
  }
}
